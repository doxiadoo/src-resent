import os, time, discord, asyncpg, random, string
...
from utils.ext import Client, HTTP
...
class NeoContext(commands.Context): 
 ...
 async def cmdhelp(self): 
    ...
    if command.cog_name == "owner" or "jishaku": return
 ...
 async def create_pages(self): 
  ...
  for command in self.command.commands: 
    commandname = f"{str(command.parent)} {command.name}" if str(command.parent) != "None" else command.name
    i+=1 
    embeds.append(discord.Embed(color=bot.color, title=f"{commandname}", description=command.description).set_author(name=self.author.name, icon_url=self.author.display_avatar.url if not None else '').add_field(name="usage", value=f"```{commandname} {command.usage if command.usage else ''}```", inline=False).set_footer(text=f"module: {command.cog_name} • aliases: {', '.join(a for a in command.aliases) if len(command.aliases) > 0 else 'none'} ・ {i}/{len(self.command.commands)}"))
...
class HelpCommand(commands.HelpCommand):
  ...
  async def send_group_help(self, group: commands.Group): 
   ...
   for command in group.commands: 
    commandname = f"{str(command.parent)} {command.name}" if str(command.parent) != "None" else command.name
    i+=1 
    embeds.append(discord.Embed(color=bot.color, title=f"{commandname}", description=command.description).set_author(name=ctx.author.name, icon_url=ctx.author.display_avatar.url if not None else '').add_field(name="usage", value=f"```{commandname} {command.usage if command.usage else ''}```", inline=False).set_footer(text=f"module: {command.cog_name} • aliases: {', '.join(a for a in command.aliases) if len(command.aliases) > 0 else 'none'} ・ {i}/{len(group.commands)}"))
...
class CommandClient(commands.AutoShardedBot):
    def __init__(self):
        super().__init__(command_prefix=getprefix, 
            ...
            everyone=False, users=True, replied_user=False), intents=discord.Intents.all(), 
    ...
    async def create_db_pool(self):
        self.db = await asyncpg.create_pool(port="5432", database="resent", user="postgres", host="localhost", password="admin")
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
      if isinstance(error, commands.CommandNotFound): return 
      ...
      elif isinstance(error, commands.CommandInvokeError): return await ctx.send_warning(error.original)
       ...
       await self.ext.send_error(ctx, f"An unexpected error was found. Please report the code `{key}` in our [**support server**](https://discord.gg/resent)")    
