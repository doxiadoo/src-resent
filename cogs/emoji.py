...
class emoji(commands.Cog):
   ...
   async def steal_ctx(self, ctx: commands.Context) -> Optional[List[Union[StolenEmoji, discord.StickerItem]]]:
        ...
        if not reference:
            await ctx.send_warning("Reply to a message with this command to steal an emoji, or run ``addemoji``.")
            return None
   ...
   async def steal(self, ctx: commands.Context, *names: str):
        ...
        if isinstance(emojis[0], discord.StickerItem):
            ...
            try:
                ...
                return await ctx.send_warning(f"{STICKER_FAIL}, {type(error).__name__}: {error}")
        ...
        async with aiohttp.ClientSession() as session:
            for emoji, name in zip_longest(emojis, names):
                ...
                try:
                    ...
                    return await ctx.send_warning(f"{EMOJI_FAIL} {emoji.name}, {type(error).__name__}: {error}")
                try:
