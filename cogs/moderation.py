...
class moderation(commands.Cog): 
  ...
  async def nickname(self, ctx: commands.Context, target: discord.Member, *, nick: str = None):
    ...
    if nick == None or nick.lower() == "none": return await ctx.send_success(f"Cleared **{target.name}'s** nickname")
    await target.edit(nick=nick)
    return await ctx.send_success(f"Changed **{target.name}'s** nickname to **{nick}**")    
  ...
  async def kick(self, ctx: commands.Context, target: discord.Member, *, reason: str='No Reason Provided'):
    ...
    if not await InvokeClass.invoke_send(ctx, target, reason): await ctx.send_success(f"**{target}** has been kicked | {reason}")
  ...
  async def ban(self, ctx: commands.Context, target: Union[discord.Member, discord.User], *, reason: str='No Reason Provided'):
      ...
      if check is not None: return await ctx.send_warning(f"{target.mention} is globalbanned, they cannot be unbanned.")
      ...
      if not await InvokeClass.invoke_send(ctx, target, reason): await ctx.send_success(f"**{target}** has been banned | {reason}")
  ...
  async def mute(self, ctx: commands.Context, member: discord.Member, time: ValidTime=int, *, reason: str="No reason provided"): 
      ...
      if member.is_timed_out(): return await ctx.send_error(f"{member.mention} is **already** muted")
      if member.guild_permissions.administrator: return await ctx.send_warning("You **cannot** mute an administrator")
      ...
      if not await InvokeClass.invoke_send(ctx, member, reason): await ctx.send_success(f"**{member}** has been muted for {humanfriendly.format_timespan(time)} | {reason}")
  ...
  async def unban(self, ctx: commands.Context, member: discord.User, *, reason: str="No reason provided"):
    ...
    if check is not None: return await ctx.send_warning(f"{member.mention} is globalbanned, they cannot be unbanned.")
    try:
  ...
  async def strip(self, ctx: commands.Context, member: discord.Member, *, reason: str='No reason provided'):
     if ctx.author.top_role <= member.top_role and ctx.author.id not in self.bot.owner_ids:
            return await ctx.send_warning("You can't **strip** someone above you or someone with the same role as you.")
  ...
  async def role_remove(self, ctx: commands.Context, user: discord.Member, *, role: GoodRole):
    if role.is_premium_subscriber(): return await ctx.send_warning('I cant remove integrated roles from users.')
  ...
  async def rolehumansremove(self, ctx: commands.Context, *, role: GoodRole):
      if self.bot.ext.is_dangerous(role): return await ctx.send_warning('I cant remove roles from users that have dangerous permissions.')
      if role.is_premium_subscriber(): return await ctx.send_warning('I cant remove integrated roles from users.')
  ...
  async def rolebotsremove(self, ctx: commands.Context, *, role: GoodRole):
      if self.bot.ext.is_dangerous(role): return await ctx.send_warning('I cant remove roles from bots that have dangerous permissions.')
      if role.is_premium_subscriber(): return await ctx.send_warning('I cant remove integrated roles from bots.')
  ...
  async def rolebotsadd(self, ctx: commands.Context, *, role: GoodRole):  
    if self.bot.ext.is_dangerous(role): return await ctx.send_warning('I cant remove roles from users that have dangerous permissions.')
    if role.is_premium_subscriber(): return await ctx.send_warning('I cant assign integrated roles to bots.')
  ...
  async def autokick_add(self, ctx: commands.Context, *, member: Union[discord.Member, discord.User]): 
      if isinstance(member, discord.Member) and not Permissions.check_hierarchy(self.bot, ctx.author, member): return await ctx.send_warning(f"You cannot autokick*{member.mention}")
      check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
      if check is not None: return await ctx.send_warning(f"{member.mention} is globalbanned, they cannot be autokicked.")
      che = await self.bot.db.fetchrow("SELECT * FROM autokick WHERE guild_id = {} AND autokick_users = {}".format(ctx.guild.id, member.id))
      if che is not None: return await ctx.send_warning(f"**{member}** is already on the autokick list.")
  ...
  async def autokick_remove(self, ctx: commands.Context, *, member: discord.User): 
    check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
    if check is not None: return await ctx.send_warning(f"{member.mention} is globalbanned, they cannot be removed from autokick list.")    
    che = await self.bot.db.fetchrow("SELECT * FROM autokick WHERE guild_id = {} AND autokick_users = {}".format(ctx.guild.id, member.id))      
    if che is None: return await ctx.send_warning(f"**{member}** is not on autokick list.") 
  ...
  async def private_unwhitelist(self, ctx: commands.Context, *, member: discord.User): 
      check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
      if check is not None: return await ctx.send_warning(f"{member.mention} is globalbanned, they cannot be private unwhitelisted.")    
      che = await self.bot.db.fetchrow("SELECT * FROM private WHERE guild_id = {} AND private_users = {}".format(ctx.guild.id, member.id))      
      if che is None: return await ctx.send_warning(f"**{member}** is not on private whitelist.") 
  ...
  async def private_whitelist(self, ctx: commands.Context, *, member: Union[discord.Member, discord.User]): 
      check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
      if check is not None: return await ctx.send_warning(f"{member.mention} is globalbanned, they cannot be private whitelisted.")
      che = await self.bot.db.fetchrow("SELECT * FROM private WHERE guild_id = {} AND private_users = {}".format(ctx.guild.id, member.id))
      if che is not None: return await ctx.send_warning(f"**{member}** is already on the private whitelist.")
  ...
  async def private_enable(self, ctx: commands.Context):
      ...
      if private is not None: return await ctx.send_warning(f"server has already been set to private")
      await self.bot.db.execute("INSERT INTO private VALUES ($1)", ctx.guild.id)
      return await ctx.send_success('marked the server as private, new members will be automatically kicked.') 
  ...
  async def private_disable(self, ctx: commands.Context):
      private = await self.bot.db.fetchrow("SELECT * FROM private WHERE guild_id = $1", ctx.guild.id)
      if private is None: return await ctx.send_warning(f"server is not set to private")
      await self.bot.db.execute("DELETE FROM private WHERE guild_id = $1", ctx.guild.id)
      return await ctx.send_success('unmarked the server as private, new members will not be kicked.') 
