...
def has_admin():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        if not admin: return await ctx.send_warning("You do not have **anti-nuke admin**")
        return True
...
def is_enabled():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        if not module:
            return await ctx.send_warning("AntiNuke is not **enabled** in this server. Use `antinuke enable` to **enable** it.")
        return True
...
class antinuke(commands.Cog):
    ...
    @has_admin()
    @commands.group(invoke_without_command=False, description='Anti-Nuke Commands', help="<subcommand>", usage="settings",  brief="Anti-Nuke Admin", aliases=['an'])
    ...
    async def whitelist(self, ctx: commands.Context, user: discord.User):
        ...
        if whitelist: 
            ...
            return await ctx.send_warning(f"**{user.name}** has been **unwhitelisted** in this server.")
        
        await ctx.bot.db.execute("INSERT INTO antinuke_whitelist VALUES ($1, $2)", ctx.guild.id, user.id)
        
        return await ctx.send_success(f"**{user.name}** has been **whitelisted** in this server.")
    ...
    async def admin(self, ctx: commands.Context, user: discord.User):
        ...
        if admin: 
            ...
            return await ctx.send_warning(f"**{user.name}** has been **removed** from the **Anti-Nuke Admin** list in this server.")
        
        await ctx.bot.db.execute("INSERT INTO antinuke_admins VALUES ($1, $2)", ctx.guild.id, user.id)
        
        return await ctx.send_success(f"**{user.name}** has been **added** to the **Anti-Nuke Admin** list in this server.")
    ...
    async def whitelisted(self, ctx: commands.Context):
        ...
        if not whitelisted: return await ctx.send_warning("No users are **whitelisted** in this server.")
    ...
    async def admins(self, ctx: commands.Context):
        ...
        if not admins: return await ctx.send_warning("No users are **Anti-Nuke Admins** in this server.")
    ...
    async def enable(self, ctx: commands.Context):
        ...
        if enabled: return await ctx.send_warning("AntiNuke is already **enabled** in this server.")
        ...
        return await ctx.send_success("Anti-Nuke has been **enabled** in this server.")
    ...
    async def disable(self, ctx: commands.Context):
        ...
        return await ctx.send_success("Anti-Nuke has been **disabled** in this server.")
    ...
    async def toggle(self, ctx: commands.Context, module: str):
        if module.capitalize() not in self.modules: return await ctx.send_warning(f"The module `{module}` is not a valid **Anti-Nuke module**.")
        ...
        return await ctx.send_success(f"Anti-Nuke module `{module}` has been **{'Enabled' if an_module.toggled else 'Disabled'}**.")
    ...
    async def threshold(self, ctx: commands.Context, module: str, threshold: int):
        ...
        return await ctx.send_success(f"Anti-Nuke module `{module}` threshold has been set to `{threshold}`.")
    ...
    async def action(self, ctx: commands.Context, module: str, action: str):
        if module.capitalize() not in self.modules: return await ctx.send_warning(f"The module `{module}` is not a valid **Anti-Nuke module**.")
        if action.lower() not in ['ban', 'warn', 'kick' ,'strip']: return await ctx.send_warning(f"The action `{action}` is not a valid action. Use `ban`, `warn`, `kick` or 'strip'.")
        
        an_module = await AntiNukeModule.from_database(self.bot.db, ctx.guild.id, module.capitalize())
        an_module.punishment = action
        await an_module.update(self.bot.db, ctx.guild.id)
        
        return await ctx.send_success(f"Anti-Nuke module `{module}` action has been set to `{action}`.")
