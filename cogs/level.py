...
class leveling(commands.Cog): 
  ...
  async def add(self, ctx: commands.Context, level: int, *, role: discord.Role): 
    if self.bot.ext.is_dangerous(role): return await ctx.send_warning('You cannot make a level role a role with dangerous permissions.')
  ...
  async def leaderboard(self, ctx: commands.Context):
    ...
    if len(results) == 0: return await ctx.send_error("nobody is on the **level leaderboard**")
    def sortkey(e): 
      return int(e[4])
  ...
  async def channel(self, ctx: commands.Context, *, channel: discord.TextChannel): 
      ...
      if check is None: return await ctx.send_warning("leveling system is not enabled")
      if channel is None: 
       ...
       await ctx.send_success(f"set the channel for leveling system to {channel.mention}")   
