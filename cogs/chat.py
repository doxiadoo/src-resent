...
class chat(commands.Cog): 
  ...
  async def ar_list(self, ctx: commands.Context): 
        ...
        if not results: return await ctx.send_warning(f"No **autoresponders** are set.")
  ...
  async def delete(self, ctx: commands.Context, *, toggle: str):
        ...
        if not key: return await ctx.send_warning(f"You don't have an autoresponse for `{toggle}`")
        await ctx.bot.db.execute("DELETE FROM autoresponses WHERE guild_id = $1 AND key = $2", ctx.guild.id, toggle)
        return await ctx.send_success(f'Successfully **deleted** the autoresponse for `{toggle}`')
  ...
  async def add(self, ctx: commands.Context, *, message: str):
        ...
        if key: return await ctx.send_warning(f"You already have an autoresponse for `{autoresponse[0]}`")
        await ctx.bot.db.execute("INSERT INTO autoresponses VALUES ($1, $2, $3)", ctx.guild.id, autoresponse[0], autoresponse[1].lstrip())
        return await ctx.send_success(f'Successfully **added** the autoresponse for `{autoresponse[0].lstrip()}` with response `{autoresponse[1].lstrip()}`')
  ...
  async def autoreact_list(self, ctx: commands.Context): 
      ...
      if len(check) == 0: return await ctx.send_warning("this server has no **autoreactions**")
      i=0
      k=1
      l=0
      mes = ""
