...
class music(commands.Cog):
    ...
    async def play(self, ctx: commands.Context, *, query: str):
     ...
     if check is None: return await ctx.send_warning('You are not in a voice channel.')

     if not ctx.voice_client: player = await ctx.author.voice.channel.connect(cls=Player, self_deaf=True)
     else: player: Player = ctx.voice_client
  
     player.set_context(ctx) 
     player.awaiting = True
    
     try: results = await player.get_tracks(query=query, ctx=ctx)
     except Exception as e: return await ctx.send_warning("There was an issue fetching that track.")
     if not results: await ctx.send_warning("No song found")
