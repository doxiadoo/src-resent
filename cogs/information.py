...
class information(commands.Cog):
    ...
    @commands.command(aliases=["pfp", "uav", "avatar"], description="see user's avatar", help="information", usage="<user>")
    ...
    async def serveravatar(self, ctx: commands.Context, *, member: discord.Member = None):
      if member is None: member = ctx.author
      if member.guild_avatar is None: return await ctx.send_warning(f'**{member}** doesnt have a server avatar set.')
    ...
    async def guildsplash(self, ctx, *, link: str):
      ...
      if data == None: return await ctx.send_warning("this server doesn't have a splash set.")
    ...
    async def status(self, ctx: commands.Context, member: discord.Member):
        ...
        for s in member.activities:
            if isinstance(s, discord.CustomActivity):
                embed = discord.Embed(title='status', description=f'{member.CustomActivity}')
