 ...
 {guild.boost_tier} - returns the server's boost level
   """
    ...
    async def autorole_add(self, ctx: Context, *, role: Union[Role, str]): 
      ...
      if self.bot.ext.is_dangerous(role): return await ctx.send_warning("I cannot assign roles with dangerous permissions as autorole.")
    ...
    async def add(self, ctx: commands.Context, permissions: str, role: discord.Role):
      if permissions not in self.perms: return await ctx.send_warning('that is not a valid **fakepermission**.')
    ...
    async def stickymessage_add(self, ctx, channel: typing.Optional[discord.TextChannel] = None, *, key: str):
        if channel is None:
          channel = ctx.channel
        if await ctx.bot.db.fetch("SELECT * FROM stickym WHERE guild_id = $1 AND channel_id = $2", ctx.guild.id, channel.id): return await ctx.send_warning(f"You already have a sticky message for {channel.mention}")
    ...
    async def stickymessage_config(self, ctx: commands.context):
        ...
        if not results:
            return await ctx.send_warning("There is no sticky message configured in this server")
        embeds = [
