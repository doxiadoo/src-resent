...
class misc(commands.Cog):
  ...
  @Permissions.has_permission(manage_guild=True)
  async def webhook_send(self, ctx: ResentContext, code: ValidWebhookCode, *, script: EmbedScript=None):
  ...
  async def naughty(self, ctx):
        ...
        try:
            ...
            await ctx.send_success(f"The channel {channel.mention} has been marked as NSFW for 30 seconds.")
            await asyncio.sleep(30)
            await channel.edit(nsfw=False, reason=f'requested by {ctx.author}')
            await ctx.send_success(f"The channel {channel.mention} is no longer marked as NSFW.")
        except discord.Forbidden:
            await ctx.send_warning("I don't have the required permissions to manage channels.")
