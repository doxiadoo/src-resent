import discord, random, json, asyncio, aiohttp, requests
from discord.ext import commands
from resent import CommandClient
...
from patches.classes import RockPaperScissors, BlackTea, TicTacToe
...
class fun(commands.Cog):
    ...
    async def pack(self, ctx: commands.Context, *, member: discord.Member): 
     if member == ctx.author: return await ctx.send_warning("You **cannot** pack yourself. I don't know why you would want to either.")
    ...
    @commands.command(name='impersonate', aliases=['mock'], description='send a message as another user', brief='manage webhooks', help='fun', usage='[user] [message]')
    async def impersonate(self, ctx, user: discord.User, *, content):
      try:
          webhook = await ctx.channel.create_webhook(name=user.name, reason=f'{ctx.author} used mock command')
          await ctx.message.delete()
          await webhook.send(str(content), username=user.display_name, avatar_url=user.avatar.url)
          await webhook.delete(reason=f'{ctx.author} used mock command')
      except discord.Forbidden: 
        await ctx.send_warning("I don't have the 'manage_webhooks' permission in this channel.")
