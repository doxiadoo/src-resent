...
class settings(commands.Cog):
    ...
    async def enable(self, ctx: commands.Context):
        ...
        if social is not None and social['toggled']: return await ctx.send_warning('**Social Media** Reposting is already **enabled**.')
    ...
    async def disable(self, ctx: commands.Context):
        social = await ctx.bot.db.fetchrow('SELECT * FROM settings_social WHERE guild_id = $1', ctx.guild.id)
        if not social or not social['toggled']: return await ctx.send_warning('**Social Media** Reposting is already **disabled**.')
    ...
    async def prefix(self, ctx: commands.Context, prefix: str=None):
        social = await ctx.bot.db.fetchrow('SELECT * FROM settings_social WHERE guild_id = $1', ctx.guild.id)
        if not social: return await ctx.send_warning('Social Media Reposting is not enabled.')
        if not prefix: return await ctx.send_success(f'Current prefix: {social["prefix"]}')
        await ctx.bot.db.execute("UPDATE settings_social SET prefix = $2 WHERE guild_id = $1", ctx.guild.id, prefix)
        return await ctx.send_success(f'**social media** reposting prefix is now set to `{prefix}`.')
    ...
    async def voice(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if voice: return await ctx.send_warning('Voice logs are already enabled')
        ...
        if not voice: return await ctx.send_warning('Voice logs are already disabled')
    ...
    async def message(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if message: return await ctx.send_warning('Message logs are already enabled')
        ...
        if not message: return await ctx.send_warning('Message logs are already disabled')
    ...
    async def member(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if member: return await ctx.send_warning('member logs are already **enabled**')
        ...
        if not member: return await ctx.send_warning('member logs are already disabled')
    ...
    async def role(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if role: return await ctx.send_warning('role logs are already **enabled**')
        ...
        if not role: return await ctx.send_warning('role logs are already disabled')
    ...
    async def server(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if server: return await ctx.send_warning('server logs are already enabled')
        ...
        if not server: return await ctx.send_warning('server logs are already disabled')
    ...
    async def channel(self, ctx: commands.Context, status: bool, channel: discord.TextChannel=None):
        ...
        if status:
            if channelCheck: return await ctx.send_warning('Channel logs are already enabled')
        ...
        if not channelCheck: return await ctx.send_warning('Channel logs are already disabled')
    ...
    async def rolebutton_add(self, ctx: commands.Context, message: discord.Message, emoji: discord.Emoji, role: discord.Role):
        ...
        if message.author.id != self.bot.user.id: return await ctx.send_warning(f"I can only add role **buttons** to my own **messages**. You can create an **embed** using `{prefix}createembed (code)` and add the **button** there.")
        if self.bot.ext.is_dangerous(role): return await ctx.send_warning('I cant assign roles to users that have dangerous permissions.')
        if role.is_premium_subscriber(): return await ctx.send_warning('I cant assign integrated roles to users.')
        view = discord.ui.View()
        for component in message.components:
            if isinstance(component, discord.components.ActionRow):
                for button in component.children:
                    if button.custom_id == f"RB:{message.id}:{role.id}": return await ctx.send_warning(f"**role** {role.mention} is already **assigned** to this **message**.")
        ...
        return await ctx.send_success(f"added **role** {role.mention} to [**message**]({message.jump_url})")
    ...
    async def rolebutton_remove(self, ctx: commands.Context, message: discord.Message, role: discord.Role):
        prefix = await self.bot.get_prefix(ctx.message)
        if message.author.id != self.bot.user.id: return await ctx.send_warning(f"I can only remove role **buttons** to my own **messages**. You can create an **embed** using `{prefix}createembed (code)` and add the **button** there.")
        view = discord.ui.View()
        for component in message.components:
            if isinstance(component, discord.components.ActionRow):
                for button in component.children:
        ...
        await message.edit(view=view)
        return await ctx.send_success(f"removed **role** {role.mention} from [**message**]({message.jump_url})")
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if before.channel != after.channel and before.channel != None and after.channel != None:
            ...
            embed.set_author(name=(f'Switched Voice channels'), icon_url=member.avatar.url if member.avatar is not None else member.default_avatar.url)
            embed.add_field(name='Member:', value=f'{member.mention}', inline=True)
            embed.add_field(name='Before:', value=f'{before.channel.jump_url}', inline=True)
            embed.add_field(name='After:', value=f'{after.channel.jump_url}', inline=True)
        ...
        if before.channel != after.channel and before.channel == None:
            ...
            embed.set_author(name=(f'Joined a Voice channel'), icon_url=member.avatar.url if member.avatar is not None else member.default_avatar.url)
            embed.add_field(name='Member:', value=f'{member.mention}', inline=True)
            embed.add_field(name='Channel Category:', value=f'{channel.category}', inline=True)
            embed.add_field(name='Channel:', value=f'{after.channel.jump_url}', inline=True)
            try: await channel.send(embed=embed)
            except discord.Forbidden: return
        ...
        if before.channel != after.channel and after.channel == None:
            channel = member.guild.get_channel(voice_logs['channel_id'])
            embed = discord.Embed(color=self.bot.color, timestamp=datetime.datetime.now())
            embed.set_author(name=(f'Left a Voice channel'), icon_url=member.avatar.url if member.avatar is not None else member.default_avatar.url)
            embed.add_field(name='Member:', value=f'{member.mention}', inline=True)
            embed.add_field(name='Channel Category:', value=f'{channel.category}', inline=True)
            embed.add_field(name='Channel:', value=f'{before.channel.jump_url}', inline=True)
            try: await channel.send(embed=embed)
            except discord.Forbidden: return
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        embed.set_author(name=(f'{message.author.name} ({message.author.id}) - Deleted Message'), icon_url=message.author.avatar.url if message.author.avatar is not None else message.author.default_avatar.url)
        embed.add_field(name='Channel', value=f'{message.channel.mention}', inline=False)
        if len(message.content) > 0: embed.description = f'{message.author.mention}: {message.content}'
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        embed = discord.Embed(description=f"{after.author.mention}: {before.content}", timestamp=datetime.datetime.now(), color=self.bot.color)
        embed.set_author(name=(f'{after.author.name} ({after.author.id}) - Edited Message'), icon_url=after.author.avatar.url if after.author.avatar is not None else after.author.default_avatar.url)
        embed.add_field(name= 'After message:', value=f'[Click to see new message]({after.jump_url})')
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        embed = discord.Embed(title=f'{member.name} has joined {member.guild.name}', description=member.mention, timestamp=datetime.datetime.now(), color=self.bot.color)
        ...
        embed.add_field(name='Joined Guild', value=f"<t:{int(member.joined_at.timestamp())}:R>", inline=False)
        embed.set_footer(text=f'{member.guild.member_count} Members ﹒{member.id}', icon_url=member.avatar.url if member.avatar is not None else member.default_avatar.url)
        try: await channel.send(embed=embed)
        except discord.Forbidden: return
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        channel = member.guild.get_channel(member_logs['channel_id'])
        embed = discord.Embed(title=f'{member.name} has left {member.guild.name}', description=member.mention, timestamp=datetime.datetime.now(), color=self.bot.color)
        if member.avatar is not None: embed.set_thumbnail(url=member.avatar.url)
        embed.add_field(name='Joined Guild', value=f"<t:{int(member.joined_at.timestamp())}:R>", inline=False)
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.nick != after.nick:
            ...
            embed = discord.Embed(color=self.bot.color, timestamp=datetime.datetime.now())
            embed.set_author(name=(f'{after.name} ({after.id}) - Updated'), icon_url=after.avatar.url if after.avatar is not None else after.default_avatar.url)
    ...
    async def on_guild_role_create(self, role: discord.Role):
        ...
        async for entry in role.guild.audit_logs(limit=1, action=discord.AuditLogAction.role_create): 
            ...
            embed = discord.Embed(title=f'{role.name} ({role.id}) created', timestamp=datetime.datetime.now(), color=self.bot.color)
            embed.add_field(name='Role', value=f'{role.mention}', inline=False)
            embed.add_field(name='Created by:', value=f'{entry.user.mention}', inline=False)
            if entry.user.avatar is not None: embed.set_thumbnail(url=entry.user.avatar.url)
            embed.set_footer(icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
            try: await channel.send(embed=embed)
            except discord.Forbidden: return
    ...
    async def on_guild_role_delete(self, role: discord.Role):
        ...
        async for entry in role.guild.audit_logs(limit=1, action=discord.AuditLogAction.role_create): 
            channel = role.guild.get_channel(role_logs['channel_id'])
            embed = discord.Embed(title=f'{role.name} ({role.id}) deleted', timestamp=datetime.datetime.now(), color=self.bot.color)
    ...
    async def on_audit_log_entry_create(self, entry: discord.AuditLogEntry):
        if entry.action == discord.AuditLogAction.member_role_update:
            ...
            embed = discord.Embed(color=self.bot.color, timestamp=datetime.datetime.now())
            embed.set_author(name=(f'{entry.target.name} ({entry.target.id}) - Updated'), icon_url=entry.target.display_avatar.url)
            ...
            if entry.target.avatar is not None: embed.set_thumbnail(url=entry.target.avatar.url if entry.target.avatar is not None else entry.user.default_avatar.url)
            embed.set_footer(text=f'User ID: {entry.target.id}', icon_url=entry.target.avatar.url if entry.target.avatar is not None else entry.user.default_avatar.url)
            try: await channel.send(embed=embed)
            except discord.Forbidden: return
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        if before.name != after.name:
            async for entry in after.guild.audit_logs(limit=1, action=discord.AuditLogAction.role_update): 
                embed = discord.Embed(timestamp=datetime.datetime.now(), color=self.bot.color)
                embed.set_author(name=(f'Updated Role Name'),  icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
    ...
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        channel_logs = await self.bot.db.fetchrow("SELECT * FROM channel_logs WHERE guild_id = $1", channel.guild.id)
        if not channel_logs: return
        channel = channel.guild.get_channel(channel_logs['channel_id'])
        async for entry in channel.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
            embed = discord.Embed(timestamp=datetime.datetime.now(), color=self.bot.color)
            embed.set_author(name=(f'Channel Created'), icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        async for entry in channel.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
            embed = discord.Embed(timestamp=datetime.datetime.now(), color=self.bot.color)
            embed.set_author(name=(f'Channel Deleted'), icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
            embed.add_field(name='Deleted by:', value=f'{entry.user.mention}', inline=True)
            embed.add_field(name='Channel Category:', value=f'{channel.category}', inline=True)
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        if isinstance(after, discord.CategoryChannel): return
        
        if before.name != after.name:
            async for entry in after.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_update):
                embed = discord.Embed(timestamp=datetime.datetime.now(), color=self.bot.color)
                embed.set_author(name=(f'Updated Channel Name'), icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
                embed.add_field(name='Updated by:', value=f'{entry.user.mention}', inline=True)
                embed.add_field(name='Before Name:', value=f'{before.name}', inline=True)
                embed.add_field(name='After Name:', value=f'{after.name}', inline=True)
                try: await channel.send(embed=embed)
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        ...
        if before.name != after.name:
            async for entry in after.audit_logs(limit=1, action=discord.AuditLogAction.guild_update):
                embed = discord.Embed(timestamp=datetime.datetime.now(), color=self.bot.color)
                embed.set_author(name=(f'Updated Server Name'), icon_url=entry.user.avatar.url if entry.user.avatar is not None else entry.user.default_avatar.url)
                embed.add_field(name='Updated by:', value=f'{entry.user.mention}', inline=True)
                embed.add_field(name='Before Name:', value=f'{before.name}', inline=True)
                embed.add_field(name='After Name:', value=f'{after.name}', inline=True)
                try: await channel.send(embed=embed)
