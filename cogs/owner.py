import discord, datetime, aiohttp, asyncio, textwrap, os
from discord.utils import get
from discord.ext import commands
from discord.ext.commands import Context
from discord import Embed
from typing import Optional
from patches.classes import TimeConverter
...
class owner(commands.Cog):
   ...
   @commands.is_owner()
   @donor.command(name='add', description="add a user to donors", usage="[member id]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @donor.command(name='remove', description="remove a user from donor", usage="[member id]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(aliases=["guilds"], name='servers', description="list all the servers resent is in", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='portal', description="get an invite to a guild", usage="[guild id]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='unblacklist', description="unblacklist a user", usage="[member id]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='globalban', description="ban a user from all servers", usage="[user]", help='owner', brief="bot owner")
   async def globalban(self, ctx: commands.Context, *, member: discord.User): 
    if member.id in OWNERS: return await ctx.send_warning("do not globalban a bot owner, retard.")
    if member.id == ctx.bot.user.id: return await ctx.send_warning("do not globalban me retard.")
    check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
    if check is not None: return await ctx.send_warning(f"{member.mention} is already globalbanned.")
   ...
   @commands.is_owner()
   @commands.command(name='unglobalban', description="unban a user from all servers", usage="[user]", help="owner", brief="bot owner")
   async def unglobalban(self, ctx: commands.Context, *, member: discord.User): 
    check = await self.bot.db.fetchrow("SELECT * FROM globalban WHERE banned = $1", member.id) 
    if check is None: return await ctx.send_warning(f"{member.mention} isn't globalbanned.")
    guild = discord.Guild
   ...
   @commands.is_owner()
   @commands.command(name='blacklist', description="blacklist a user from the bot", brief="owner", usage="[user]", help="owner")
   async def blacklist(self, ctx: commands.Context, *, member: discord.User): 
      if member.id in OWNERS: return await ctx.send_warning("do not blacklist a bot owner, retard.")
   ...
   @commands.is_owner()
   @commands.command(name='pingall', description="ping all members", help="owner", brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(description='set bot pfp', usage='[image url | file]', help='owner', brief='bot owner')
   async def botpfp(self, ctx: commands.Context, *, image: str=None):
        ...
        if isinstance(image, str):
            ...
            if not image.content_type.startswith('image/'):
                return await ctx.send_warning('Invalid file type. Please provide an image file.')
   ...
   @commands.is_owner()
   @commands.command(description='set bot banner', usage='[image_url | file]', help='owner', brief='bot owner')
   ...
   @commands.is_owner()
   @commands.command(name='sharedservers', description="show servers a user shares with the bot", usage="[user]", help='owner', brief="bot owner")
   async def sharedservers(self, ctx, user: discord.User = None): 
            ...
            if len(lis) == 0: return await ctx.send_warning(f"I don't share a server with {user.mention}.") 
            lis.sort(reverse=True, key=key)
            for guild in lis:
              mes = f"{mes}`{k}` {guild.name} ({guild.id}) - ({guild.member_count})\n"
              k+=1
   ...
   @commands.is_owner()
   @commands.command(name='dm', aliases=['dmu'], description="dm a user", usage="[user]", help='owner', brief="bot owner")
   async def dm(self, ctx, user: discord.User, *, message: str):
        destination = get(self.bot.get_all_members(), id=user.id)
        if not destination:
            return await ctx.send_warning("Invalid ID or user not found. You can only send messages to people I share a server with.")
        await destination.send(message)
        await ctx.send_success(f"Sent direct message to {user.mention}.")

   @commands.is_owner()
   @commands.command(name='say', description="have the bot say something", usage="[message]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='spam', description="have the bot spam something", usage="[channel] [amount] [message]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='pingu', description="have the bot spam something and delete it", usage="[channel] [amount] [message]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(name='pingall', description="have the bot ping everyone", help='owner', brief="bot owner")
   async def pingall(self, ctx: commands.Context):
        guild: discord.Guild = ctx.guild
        mentions = " ".join(m.mention for m in guild.members if not m.bot)
        await ctx.message.delete()
   ...
   @commands.is_owner()
   @commands.command(name='leaveg', description="have the bot leave a guild", usage="[guild id]", help='owner', brief="bot owner")
   ...
   @commands.is_owner()
   @commands.command(aliases=["gblacklist"], name='blacklistg', description="blacklist a guild", usage="[guild id]", help='owner', brief="bot owner")
   async def blacklistg(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM gblacklist WHERE guild_id = $1", guild) 
        if check is not None: return await ctx.send_warning(f"this guild is **already** blacklisted.")
   ...
   @commands.is_owner()
   @commands.command(aliases=["gunblacklist"], name='unblacklistg', description="unblacklist a guild", usage="[guild id]", help='owner', brief="bot owner")
   async def unblacklistg(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM gblacklist WHERE guild_id = $1", guild) 
        if check is None: return await ctx.send_warning(f"this guild is **not** blacklisted.")
        await self.bot.db.execute("DELETE FROM gblacklist WHERE guild_id = {}".format(guild))   
        await ctx.send_success("the guild has been **unblacklisted**.")

   @commands.is_owner()
   @commands.command(aliases=["gwhitelist"], name='whitelistg', description="whitelist a guild", usage="[guild id]", help='owner', brief="bot owner")
   async def whitelistg(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM gwhitelist WHERE guild_id = $1", guild) 
        if check is not None: return await ctx.send_warning(f"this guild is **already** whitelisted.")
        await self.bot.db.execute("INSERT INTO gwhitelist VALUES ($1)", guild)
        await ctx.send_success("the guild has been **whitelisted**.")

   @commands.is_owner()
   @commands.command(aliases=["gunwhitelist"], name='unwhitelistg', description="unwhitelistg a guild", usage="[guild id]", help='owner', brief="bot owner")
   async def unwhitelistg(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM gwhitelist WHERE guild_id = $1", guild) 
        if check is None: return await ctx.send_warning(f"this guild is **not** whitelisted.")
        await self.bot.db.execute("DELETE FROM gwhitelist WHERE guild_id = {}".format(guild))   
        await ctx.send_success("the guild has been **unwhitelistlisted**.") 

   @commands.is_owner()
   @commands.command(aliases=["mwhitelist"], name='whitelistm', description="make a guild immune to perm checks", usage="[guild id]", help='owner', brief="bot owner")
   async def whitelistm(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM mwhitelist WHERE guild_id = $1", guild) 
        if check is not None: return await ctx.send_warning(f"this guild is **already** whitelisted.")
        await self.bot.db.execute("INSERT INTO mwhitelist VALUES ($1)", guild)
        await ctx.send_success("the guild has been **whitelisted**.")
   
   @commands.is_owner()
   @commands.command(aliases=["munwhitelist"], name="unwhitelistm", description="remove a guild from being immune to perm checks", usage="[guild id]", help='owner', brief="bot owner")
   async def unwhitelistm(self, ctx, guild: int):
        check = await self.bot.db.fetchrow("SELECT * FROM mwhitelist WHERE guild_id = $1", guild) 
        if check is None: return await ctx.send_warning(f"this guild is **not** whitelisted.")
        await self.bot.db.execute("DELETE FROM mwhitelist WHERE guild_id = {}".format(guild))   
        await ctx.send_success("the guild has been **unwhitelisted**.") 

   @commands.is_owner()
   @commands.command(name="forcemarry", description="forcemarry two users together", usage="[member id] [member id]", help='owner', brief="bot owner")
   async def forcemarry(self, ctx, member1:int, member2:int):
       await self.bot.db.execute("INSERT INTO marry VALUES ($1, $2, $3)", member1, member2, datetime.datetime.now().timestamp())   
       await ctx.send_success("successfuly forcemarriaged.")

   @commands.is_owner()
   @commands.command(name="forcedivorce", description="forcedivorce two users", usage="[member id] [member id]", help='owner', brief="bot owner")
   async def forcedivorce(self, ctx, member1:int, member2:int):
       await self.bot.db.execute("DELETE FROM marry WHERE author = {} AND soulmate = {}".format(member1, member2))
       await ctx.send_success("successfuly forcedivorced.")

   @commands.is_owner()
   @commands.command(aliases=["gg"], description="show information about a server", help='owner', brief="bot owner")
   async def getguild(self, ctx: Context, guild:int):
        guild = self.bot.get_guild(int(guild))   
        if guild == None: return await ctx.send_warning('no guild found for that id.')
        ...
        embed = Embed(color=self.bot.color, title=f"{guild.name}", timestamp=datetime.datetime.now(), description=f"Server created on {self.TimeConverter.convert_datetime(guild.created_at.replace(tzinfo=None))}\n{desc}")   
   ...
   async def guwulock(self, ctx: commands.Context, *, member: discord.User, reason: str="No reason provided."): 
    if member.id in OWNERS: return await ctx.send_warning("do not global uwulock a bot owner, retard.")
    if member.id == ctx.bot.user.id: return await ctx.send_warning("do not global uwulock me retard.")
    ...
    if check is None: await ctx.send_success(f"**{member}** has been globaluwulocked | {reason}")
    else: await ctx.send_success(f"**{member}** has been globalunlocked | {reason}")
