...
from patches.classes import PositionConverter
...
class edit(commands.Cog):
      ...
      if len(ctx.message.attachments) > 0:
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    ...
                    return await ctx.send_warning('something went wrong when trying to get the image.')
      else:
    ...
    @Permissions.has_permission(manage_roles=True) 
    async def re_icon(self, ctx: commands.Context, role: GoodRole, emoji: Union[discord.PartialEmoji, str]):      
    ...
    async def ve_userlimit(self, ctx: commands.Context, channel: discord.VoiceChannel, user_limit: int):
        ...
        await ctx.send_success(f'user limit for {channel.mention} has been set to {user_limit}.')
